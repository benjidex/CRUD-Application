...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        setError('Cannot connect to server. Start backend on http://localhost:4000');
        setLoading(false);
        return;
      }
  ...
  }, [filters, search, serverOnline, fetchTasks]);
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      {showForm && <TaskForm onSubmit={handleCreate} onCancel={() => setShowForm(false)} loading={formLoading} />}
      {editTask && <TaskForm initial={editTask} onSubmit={handleUpdate} onCancel={() => setEditTask(null)} loading={formLoading} />}
      ...
      <main style={{ maxWidth: '1200px', margin: '0 auto', padding: '32px 32px' }}>
        ...
        {error && (
          ...
          >
            <span>{error}</span>
            <button onClick={() => setError('')} style={{ color: 'var(--red)', fontSize: '1rem' }}>
              ✕
            </button>
          </div>
        )}
        ...
        {loading ? (
          ...
          >
            ...
            <p style={{ color: 'var(--text-faint)', fontSize: '0.85rem', marginBottom: '20px' }}>
              {search || filters.status !== 'all' || filters.priority !== 'all' ? 'Try clearing your filters' : 'Create your first task to get started'}
            </p>
